...
##prepare scATAC-seq data from bam files
atac_in <- ENCLfunc("/PATH_TO_BAM",ENCL2000,type="paired")

##read in scRNA-seq data with ensembl id as rownames, see example data "expr_example" for reference
expr_in <- read.csv(file="scRNA_data.csv")

##load built-in training data
DNase_train <- DNase_hg19_cluster_2000
RNA_train <- RNA_hg19
